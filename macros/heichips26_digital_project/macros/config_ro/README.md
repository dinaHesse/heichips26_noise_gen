# ihp-sg13cmos5l config_ro

<p align="center">
  <a href="final/render/config_ro.png">
    <img src="final/render/config_ro.png" alt="Render of the ihp-sg13cmos5l config_ro layout" width=50%>
  </a>
  <br>
  <em>Render of the ihp-sg13cmos5l config_ro layout.</em>
</p>
...
│     └─ config_ro.vh
├─ 📁 flow/
│  ├─ 📁 final/               # .gitignore'd — important files are copied to config_ro/final/ (listed here to document LibreLane output folders)
...
```sh
make lint-verilog                # lint the config_ro design
make lint-verilog CELL=config_ro   # equivalent: CELL defaults to config_ro
make lint-verilog-all            # lint all source files
```

When `CELL=config_ro` (the default), all synthesis sources are passed to Verilator.
...
The simulation targets are unified and accept an optional `CELL` variable (default: `config_ro`).
...
> [!NOTE]
> In the current repository state, the provided Verilog, cocotb, and Xschem testbench/viewer files are for `config_ro`.
...
When `CELL=config_ro` (the default), the full `MODULES_SIM` source list and the `.sv` testbench are selected automatically.
...
The waveform is written to `testbenches/verilog/` (e.g. `testbenches/verilog/config_ro_tb.fst`):

```sh
make sim-rtl-verilog              # run config_ro RTL simulation
```

To view the waveform afterwards:

```sh
make sim-view-verilog                                  # view config_ro waveform
make sim-view-verilog WAVEFORM_VIEWER=surfer           # use Surfer instead
```

The simulation folder contains a pre-configured waveform layout file (`config_ro_tb.gtkw` for GTKWave, `config_ro_tb.surf.ron` for Surfer).
...
The cocotb testbench is located in `testbenches/cocotb/config_ro_tb.py` and exercises:

- reset clears the config_ro to 0
- the config_ro holds its value while `enable_i` is low
- the config_ro increments by 1 on every rising clock edge while `enable_i` is high
- the config_ro wraps from `CTR_MAX` back to 0

```sh
make sim-rtl-cocotb               # run config_ro RTL cocotb simulation
```

To run the gate-level (GL) cocotb simulation (sources the post-synthesis netlist from `final/nl/`):

```sh
make sim-gl-cocotb                # gate-level simulation of config_ro
```

> [!NOTE]
> Gate-level simulation requires the latest implementation in `flow/final/` (and a `final/nl/config_ro.nl.v` copy via `make copy-final`).

A waveform file is generated under `testbenches/cocotb/sim_build/config_ro.fst`.
To view it:

```sh
make sim-view-cocotb                                  # view config_ro waveform
make sim-view-cocotb WAVEFORM_VIEWER=surfer           # use Surfer instead
```

The cocotb folder contains a pre-configured waveform layout file (`config_ro_tb.gtkw` for GTKWave, `config_ro_tb.surf.ron` for Surfer).
The view target loads it automatically together with the current `.fst`, so signal formatting is preserved across runs.

#### Gate-Level Xschem Simulation

> [!TIP]
> This gate-level flow brings the hardened digital macro into Xschem as an XSPICE model, so it can be simulated together with analog circuitry in ngspice. This is what enables **analog mixed-signal designs** in Xschem. Instantiate the `config_ro` symbol next to your analog blocks in a testbench schematic and simulate the whole system in one run.

Runs the mixed-signal gate-level transient simulation testbench in `testbenches/xschem/<CELL>_tb_tran.sch`:

```sh
make sim-gl-xschem                # run config_ro gate-level Xschem simulation
...
```sh
make sim-view-xschem              # plot config_ro simulation results (default: plot_config_ro)
...
1. `sim-rtl-verilog` (default: `config_ro`)
2. `sim-rtl-cocotb` (default: `config_ro`)
3. `sim-gl-cocotb` (default: `config_ro`)
4. `sim-gl-xschem` (default: `config_ro`)
...
The FPGA flow emulates `config_ro` standalone (native `clk_i`/`rst_ni`/`enable_i`/`count_o` ports, no chip-level wrapper) and targets a [ULX3S](https://radiona.org/ulx3s/) board by default (ECP5, Yosys → nextpnr-ecp5 → ecppack), flashed with `openFPGALoader`.
...
> [!NOTE]
> The Xschem testbench `.include`s the XSPICE model `netlist/xspice/config_ro.xspice`. Directly after `make clean`, run `make build-top` (or the full `make all`) once before `make sim-gl-xschem`, otherwise the include fails.
