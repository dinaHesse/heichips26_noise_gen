# FPGA Emulation Flow — `config_ro` (standalone)

Emulates the `config_ro` macro on its own — native ports (`clk_i`,
...
Each board's constraint file maps `config_ro`'s native ports onto pins
