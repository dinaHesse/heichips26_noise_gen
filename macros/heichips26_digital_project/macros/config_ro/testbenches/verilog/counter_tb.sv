...
// Description: SystemVerilog testbench for the config_ro module.
...
module config_ro_tb;
  ...
  initial begin
    ...
    if (count !== '0)
      $fatal(1, "FAIL: config_ro not zero after reset (got %0d)", count);

    // Hold disabled for a few cycles; value must not change
    #(5 * CLK_PERIOD_NS);
    if (count !== '0)
      $fatal(1, "FAIL: config_ro changed while disabled (got %0d)", count);

    // Enable config_ro, run for one full wrap and a few extra cycles
    enable = 1'b1;
