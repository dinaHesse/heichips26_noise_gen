...
async def start_up(dut):
    """Startup sequence: clock + reset, config_ro disabled, value cleared."""
...
@cocotb.test()
async def test_reset_clears_config_ro(dut):
    """After reset deasserts, count_o must be zero."""
    logger = logging.getLogger("config_ro_tb")

    logger.info("Startup sequence...")
    await start_up(dut)

    assert int(dut.count_o.value) == 0, \
        f"config_ro not zero after reset (got {int(dut.count_o.value)})"
...
async def test_holds_when_disabled(dut):
    ...
    assert int(dut.count_o.value) == 0, \
        f"config_ro changed while disabled (got {int(dut.count_o.value)})"
...
async def test_wraps_at_max(dut):
    """config_ro must wrap from CTR_MAX back to 0."""
    logger = logging.getLogger("config_ro_tb")
    ...
    assert saw_max,  "config_ro never reached CTR_MAX"
    assert saw_wrap, "config_ro did not wrap from CTR_MAX to 0"

    logger.info("Done!")


def config_ro_runner():
    ...
    if gl:
        ...
        sources.append(proj_path / "../../rtl/config_ro.sv")
