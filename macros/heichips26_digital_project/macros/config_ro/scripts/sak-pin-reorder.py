...
Two netlist formats are handled (auto-detected, see ``--format``):
  ...
  - **xspice**: a digital XSPICE model generated by ``spi2xspice.py``,
    ...
    with bus brackets flattened (``config_ro_value_o[0]`` ->
    ``a_config_ro_value_o_0_``).
...
a bare base (``config_ro_value_o``) or with a range; the symbol bus
...
def sim_pinname_to_xspice(real: str) -> str:
    ...
    so ``clock_i`` -> ``a_clock_i`` and ``config_ro_value_o[0]`` ->
    ``a_config_ro_value_o_0_``.
    """
